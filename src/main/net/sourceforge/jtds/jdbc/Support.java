...
// VM 1.4+ only             return new java.sql.Time(cal.getTimeInMillis());*/
     ...
     * @throws XAException if an error condition occurs
     */
    public static void xa_start(Connection connection, int xaConId, Xid xid, int flags)
        ...
        try {
            ...
            if (args[0] == XAResource.XA_OK && cookie != null) {
                ((ConnectionJDBC2) connection).enlistConnection(cookie);
            }
    ...
    public static Xid[] xa_recover(Connection connection, int xaConId, int flags)
        ...
        try {
            ...
            if (args[0] >= 0) {
                int n = buffer.length / JtdsXid.XID_SIZE;
                list = new JtdsXid[n];
                for (int i = 0; i < n; i++) {
                    list[i] = new JtdsXid(buffer, i * JtdsXid.XID_SIZE);
                }
            }
        } catch (SQLException e) {
            raiseXAException(e);
     ...
     * @throws XAException exception derived from the code>SQLException</code>
     */
    ...
    /**
     * Construct and throw an <code>XAException</code> with an explanatory message and the XA error
     * code set.
     *
     * @param errorCode the XA Error code
     * @throws XAException the constructed exception
     */

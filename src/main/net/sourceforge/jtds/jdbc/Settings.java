...
/**
 * Container for property values (settings) and their defaults.
 * <p/>
 ...
 * @version $Id: Settings.java,v 1.1 2004-08-04 01:58:39 ddkilzer Exp $
 */
public final class Settings {

    /** Default <code>appName</code> property. */
    public static final String DEFAULT_APP_NAME = "jTDS";
    ...
    public static final String DEFAULT_NAMED_PIPE_PATH_SQLSERVER = "/sql/query";
    /** Default <code>namedPipePath</code> property for Sybase. */
    public static final String DEFAULT_NAMED_PIPE_PATH_SYBASE = "/sybase/query";
    /** Default <code>packetSize</code> property for TDS 4.2 and TDS 5.0. */
    public static final int DEFAULT_PACKET_SIZE_42_50 = TdsCore.MIN_PKT_SIZE;
    /** Default <code>packetSize</code> property for TDS 7.0 and TDS 8.0. */
    public static final int DEFAULT_PACKET_SIZE_70_80 = 0; // server sets packet size
    /** Default <code>portNumber</code> property for SQL Server. */
    public static final int DEFAULT_PORT_NUMBER_SQLSERVER = 1433;
    /** Default <code>portNumber</code> property for Sybase. */
    public static final int DEFAULT_PORT_NUMBER_SYBASE = 7100;
    /** Default <code>prepareSql</code> property. */
    public static final boolean DEFAULT_PREPARE_SQL = true;
    /** Default <code>progName</code> property. */
    public static final String DEFAULT_PROG_NAME = "jTDS";
    /** Default <code>sendStringParametersAsUnicode</code> property. */
    public static final boolean DEFAULT_USE_UNICODE = true;
    ...
    public static Properties addDefaultProperties(Properties props) {
        ...
        if (props.getProperty(messageKey) == null) {
            if (serverType.equals(String.valueOf(Driver.SQLSERVER))) {
                props.setProperty(messageKey, String.valueOf(DEFAULT_PORT_NUMBER_SQLSERVER));
            }
            else if (serverType.equals(String.valueOf(Driver.SYBASE))) {
                props.setProperty(messageKey, String.valueOf(DEFAULT_PORT_NUMBER_SYBASE));
            }
        }

        messageKey = Support.getMessage("prop.databasename");
        if (props.getProperty(messageKey) == null) {
            props.setProperty(messageKey, DEFAULT_DATABASE_NAME);
        }

        messageKey = Support.getMessage("prop.appname");
        if (props.getProperty(messageKey) == null) {
            props.setProperty(messageKey, DEFAULT_APP_NAME);
        }

        messageKey = Support.getMessage("prop.lastupdatecount");
        if (props.getProperty(messageKey) == null) {
            props.setProperty(messageKey, String.valueOf(DEFAULT_LAST_UPDATE_COUNT));
        }

        messageKey = Support.getMessage("prop.lobbuffer");
        if (props.getProperty(messageKey) == null) {
            props.setProperty(messageKey, String.valueOf(DEFAULT_LOB_BUFFER_SIZE));
        }

        messageKey = Support.getMessage("prop.logintimeout");
        if (props.getProperty(messageKey) == null) {
            props.setProperty(messageKey, String.valueOf(DEFAULT_LOGIN_TIMEOUT));
        }

        messageKey = Support.getMessage("prop.macaddress");
        if (props.getProperty(messageKey) == null) {
            props.setProperty(messageKey, DEFAULT_MAC_ADDRESS);
        }

        messageKey = Support.getMessage("prop.namedpipe");
        if (props.getProperty(messageKey) == null) {
            props.setProperty(messageKey, String.valueOf(DEFAULT_NAMED_PIPE));
        }
        ...
        if (props.getProperty(messageKey) == null) {
            ...
            if (tdsVersion.equals(String.valueOf(TDS_VERSION_42)) ||
                ...
                props.setProperty(messageKey, String.valueOf(DEFAULT_PACKET_SIZE_42_50));
            }
            else if (tdsVersion.equals(String.valueOf(TDS_VERSION_70)) ||
                     tdsVersion.equals(String.valueOf(TDS_VERSION_80))) {

                props.setProperty(messageKey, String.valueOf(DEFAULT_PACKET_SIZE_70_80));
            }
        }

        messageKey = Support.getMessage("prop.preparesql");
        if (props.getProperty(messageKey) == null) {
            props.setProperty(messageKey, String.valueOf(DEFAULT_PREPARE_SQL));
        }

        messageKey = Support.getMessage("prop.progname");
        if (props.getProperty(messageKey) == null) {
            props.setProperty(messageKey, DEFAULT_PROG_NAME);
        }

        messageKey = Support.getMessage("prop.useunicode");
        if (props.getProperty(messageKey) == null) {
            props.setProperty(messageKey, String.valueOf(DEFAULT_USE_UNICODE));
        }

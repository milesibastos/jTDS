...
public class ClientSideCursorTest extends DatabaseTestCase {
    ...
    public void testCachedCursor() throws Exception {
        dropTable("CachedCursorTest");
        Statement stmt = con.createStatement();
        stmt.execute("CREATE TABLE CachedCursorTest " +
                "(key1 int NOT NULL, key2 char(4) NOT NULL," +
                "data varchar(255))\r\n" +
                "ALTER TABLE CachedCursorTest " +
                "ADD CONSTRAINT PK_CachedCursorTest PRIMARY KEY CLUSTERED" +
                "( key1, key2)");
        for (int i = 1; i <= 16; i++) {
            assertEquals(1, stmt.executeUpdate("INSERT INTO CachedCursorTest VALUES(" + i + ", 'XXXX','LINE " + i + "')"));
        }
        stmt.close();
        stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
        ResultSet rs = stmt.executeQuery("SELECT * FROM CachedCursorTest ORDER BY key1");
        assertNotNull(rs);
...
//        assertTrue(rs.rowInserted());
        ...
        ResultSet rs2 = stmt2.executeQuery("SELECT * FROM CachedCursorTest ORDER BY key1");
